...
namespace WpWinNl.Utilities
{
  ...
  public static class SilverlightSerializer
  {
    ...
    {
      //var s = Convert.ToBase64String(checksum);//JSC
      var s = Convert.ToBase64String(checksum).ToCharArray();
    ...
    internal static IEnumerable<FieldInfo> GetFieldInfo(RuntimeTypeHandle itm)
    {
      lock (FieldLists)
      {
        ...
        {
          ret = FieldLists[itm] = Type.GetTypeFromHandle(itm).GetRuntimeFields().Where(p => p.IsPublic && p.GetCustomAttribute<DoNotSerialize>() == null).ToArray();
        }

        return IsChecksum ? ret.Where(p => !p.GetCustomAttributes(typeof(DoNotChecksum), true).Any()) : ret;
    ...
    private static object OldDeserializeDictionary(IDictionary o, Type itemType, BinaryReader reader)
    {
      Type keyType = null;
      Type valueType = null;
      //if (itemType.IsGenericType// JSC)
      if (itemType.GetTypeInfo().IsGenericType)
      {
        //var types = itemType.GetGenericArguments(); //JSC
        var types = itemType.GetTypeInfo().GenericTypeArguments;
        keyType = types[0];
        valueType = types[1];
      }
    ...
    private static void OldDeserializeFields(BinaryReader reader, Type itemType, object o)
    {
      ...
      for (var i = 0; i < fieldCount; i++)
      {
        ...
        var fieldType = itemType.GetRuntimeField(fieldName);
    ...
    private static void SerializeObject(Entry entry, IStorage storage)
    {
      ...
      if (IsSimpleType(itemType))
      {
        storage.WriteSimpleValue(itemType.GetTypeInfo().IsEnum ? Convert.ChangeType(item, Enum.GetUnderlyingType(itemType), CultureInfo.InvariantCulture) : item);
        return;
      }
    ...
    private static void SerializeList(IList item, Type tp, IStorage storage)
    {
      ...
      {
        var types = tp.GetTypeInfo().GenericTypeArguments;
        valueType = types[0];
      }
    ...
    internal static object CreateObject(Type itemType)
    {
      try
      {
        return itemType.GetTypeInfo().IsDefined(typeof(CreateUsingEvent), false) ? CreateInstance(itemType) : Activator.CreateInstance(itemType);
      }
      catch (Exception)
      {
        try
        {
          //var constructorInfo = itemType.GetConstructor(new Type[] { }); //JSC
    ...
    private static object CreateInstance(Type itemType)
    {
      ...
      if (ct.Instance != null && (ct.Instance.GetType() == itemType || ct.Instance.GetType().GetTypeInfo().IsSubclassOf(itemType)))
      {
        return ct.Instance;
      }
    ...
    static SilverlightSerializer()
    {
      ...
      Writers[typeof(DateTimeOffset)] = DateTimeOffsetWriter;
      ...
      Readers[typeof(DateTimeOffset)] = DateTimeOffsetReader;
    ...
    private static object DateTimeOffsetReader(BinaryReader reader)
    {
      var dateTime = new DateTime(reader.ReadInt64());
      var offset = (TimeSpan) TimeSpanReader(reader);
    ...
    private static void DateTimeOffsetWriter(BinaryWriter writer, object value)
    {
      var d = (DateTimeOffset) value;
      writer.Write(d.Ticks);
      TimeSpanWriter(writer, d.Offset);
    }
    ...
    private static void UpdateEntryWithName(Entry entry)
    {
      ...
      if (!configurations.TryGetValue(entry.Name, out entryConfiguration))
      {
        ...
        var pi = entry.OwningType.GetRuntimeProperty(entry.Name);
        if (pi != null)
        {
        ...
        {
          var fi = entry.OwningType.GetRuntimeField(entry.Name);
          if (fi != null)
          {
